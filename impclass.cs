    ...
    class impclass
    {
            ...
            try
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    if ((dt.Rows[i]["Name"].ToString() == Name && dt.Rows[i]["Writter"].ToString() == email) || (admin == true && dt.Rows[i]["Name"].ToString() == Name))
                    {
                        ...
                        BLL.Article.updateArticle(Name, Description, Type, Class, level, Site_Area, oldName, aproved);
        ...
        public void creating_ArticleDB(string Name, string Type, string Site_Area, string Description, string Class, string LVL)
        {
            try
            {
                ...
                DataTable dt = new DataTable();
                dt = BLL.Article.Load();
                int MAX = Convert.ToInt16(BLL.GoI.getMax()) + 1;
                ...
                {
                    int level = Convert.ToInt16(LVL);
                    BLL.Article.insertArticle(MAX, Name, Description, email, Site_Area, Type, level);
